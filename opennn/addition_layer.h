...
public:
                           ...
                           const bool&) override
    {
        ...
        const TensorMap<Tensor<type, Rank>, Aligned> input_1 = tensor_map<Rank>(input_views[0]);
        const TensorMap<Tensor<type, Rank>, Aligned> input_2 = tensor_map<Rank>(input_views[1]);

        TensorMap<Tensor<type, Rank>, Aligned> outputs = tensor_map<Rank>(layer_forward_propagation->outputs);
                        ...
                        unique_ptr<LayerBackPropagation>& back_propagation) const override
    {
        ...
        const TensorMap<Tensor<type, Rank>, Aligned> deltas = tensor_map<Rank>(delta_views[0]);

        TensorMap<Tensor<type, Rank>, Aligned> input_deltas_0 = tensor_map<Rank>(back_propagation->input_deltas[0]);
        TensorMap<Tensor<type, Rank>, Aligned> input_deltas_1 = tensor_map<Rank>(back_propagation->input_deltas[1]);

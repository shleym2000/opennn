...
struct DenseBackPropagationCuda : public LayerBackPropagationCuda
{
    ...
    void initialize() override
    {
        ...
        if (dense_layer->get_batch_normalization())
        {
            CHECK_CUDA(cudaMalloc(&scale_deltas_device.data, outputs_number * sizeof(float)));
            CHECK_CUDA(cudaMalloc(&offset_deltas_device.data, outputs_number * sizeof(float)));
        }
    }
    ...
    vector<TensorViewCuda*> get_tensor_views_device() override
    {
        ...
        if (dense_layer->get_batch_normalization())
        {
            views.push_back({scale_deltas_device, dense_layer->get_output_dimensions().back()});
            views.push_back({offset_deltas_device, dense_layer->get_output_dimensions().back()});
        }
    */
        vector<TensorViewCuda*> views;
        return views;
...
public:
             ...
             const string& new_label = "dense_layer")
    {
        ...
        if (batch_normalization)
        {
            offsets_device.set_descriptor({1, outputs_number, 1, 1});
            scales_device.set_descriptor({1, outputs_number, 1, 1});
        }
